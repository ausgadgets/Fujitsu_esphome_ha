...
void serialTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (millis() - lastHeartbeat > 10000) {
            ESP_LOGD("fuji", "SerialTask heartbeat #%d", ++heartbeatCounter);
        ...
        if (climate->heatPump.waitForFrame()) {
            frameProcessed = true;
            ESP_LOGD("fuji", "Frame processed in serial task");
            delay(60);  // Allow time for frame processing
            climate->heatPump.sendPendingFrame();
            climate->pendingUpdate = false;
        }
        
        // Update shared state (with shorter semaphore timeout to reduce blocking)
        if (xSemaphoreTake(climate->lock, (TickType_t)50) == pdTRUE) {
            memcpy(&(climate->sharedState), climate->heatPump.getCurrentState(),
                   sizeof(FujiFrame));
            xSemaphoreGive(climate->lock);
        }
        
        // Always yield and delay to prevent tight loops
        if (!frameProcessed) {
            delay(20);  // Longer delay when no frame to reduce CPU usage
        } else {
            delay(5);   // Shorter delay when actively processing
        }
        
        // Explicit yield to ensure other tasks can run
        taskYIELD();
    }
}
...
void FujitsuClimate::setup() {
    ...
    // Set the logging option for serial messages
    this->heatPump.setLogSerialMessages(this->log_serial_messages_);
    
    // Determine if this is a secondary controller
    bool is_secondary = (this->controller_type_ == ControllerType::SECONDARY);
    
    ESP_LOGI("fuji", "Setting up Fujitsu climate as %s controller with serial logging %s", 
             is_secondary ? "SECONDARY" : "PRIMARY",
             this->log_serial_messages_ ? "ENABLED" : "DISABLED");
    
    // Use ESPHome UART component if available, otherwise fallback to Serial2
    if (this->uart_ != nullptr) {
        ESP_LOGI("fuji", "Using ESPHome UART component");
        this->heatPump.connect(this->uart_, is_secondary);
    } else {
        ESP_LOGI("fuji", "Fallback to Serial2 with explicit pins - GPIO16 RX, GPIO17 TX");
        this->heatPump.connect(&Serial2, is_secondary, 16, 17);  // RX=16, TX=17
    }
    
    ESP_LOGD("fuji", "starting task");
    xTaskCreatePinnedToCore(serialTask, "FujiTask", 10000, (void *)this,
                            configMAX_PRIORITIES - 2, &(this->taskHandle), 1);
}
