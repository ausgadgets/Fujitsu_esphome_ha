...
#else
// For platforms that don't have HardwareSerial, we'll just forward declare it
...
class FujiHeatPump {
   private:
    esphome::uart::UARTComponent *_uart{nullptr};
    HardwareSerial *_serial{nullptr};  // Keep for backward compatibility
    byte readBuf[8];
    byte writeBuf[8];
    
    // Buffer for accumulating UART bytes
    byte uartBuffer[8];
    int uartBufferPos = 0;
   ...
   public:
    ...
    void connect(esphome::uart::UARTComponent *uart, bool secondary);
    void setLogSerialMessages(bool enable) { logSerialMessages = enable; }
