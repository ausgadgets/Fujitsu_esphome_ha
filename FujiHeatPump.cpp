...
/*and also on rabbit-aarons great work*/
...
void FujiHeatPump::encodeFrame(FujiFrame ff)
{
    ...
    writeBuf[kEnabledIndex] = (writeBuf[kEnabledIndex] & ~kEnabledMask) |
...
void FujiHeatPump::connect(esphome::uart::UARTComponent *uart, bool secondary)
{
    _uart = uart;
    _serial = nullptr;
    uartBufferPos = 0;  // Initialize buffer position
                           ...
                           int rxPin = -1, int txPin = -1)
{
    ...
    _serial->setTimeout(50);  // Reduced from 100ms to 50ms to prevent long blocking
...
void FujiHeatPump::sendPendingFrame()
{
    ...
    {
        if (_uart != nullptr) {
            // Send using ESPHome UART
            _uart->write_array(writeBuf, 8);
            _uart->flush();
        } else if (_serial != nullptr) {
            // Send using HardwareSerial (backward compatibility)
        ...
        // Clear any echo data from the buffer
        if (_uart != nullptr) {
            // For ESPHome UART, reset buffer to ignore echo
            uartBufferPos = 0;
            // Clear any available bytes (our echo)
            byte dummy;
            while (_uart->available()) {
                _uart->read_byte(&dummy);
            }
        } else if (_serial != nullptr) {
            // For HardwareSerial, read and discard echo
            byte tempBuf[8];
            if (_serial->available() >= 8) {
                _serial->readBytes(tempBuf, 8);
            }
        }
    }
}
...
bool FujiHeatPump::waitForFrame()
{
    ...
    if (millis() - lastDebugTime > 5000) {
        if (_uart != nullptr) {
            ESP_LOGD("fuji", "UART Debug: available=%d, buffer_pos=%d", 
                     _uart->available(), uartBufferPos);
        } else if (_serial != nullptr) {
            ESP_LOGD("fuji", "Serial2 Debug: available=%d", _serial->available());
        } else {
            ESP_LOGW("fuji", "No UART configured!");
        }
        lastDebugTime = millis();
    }

    if (_uart != nullptr) {
        // ESPHome UART: Read available bytes into buffer
        while (_uart->available() && uartBufferPos < 8) {
            _uart->read_byte(&uartBuffer[uartBufferPos]);
            ESP_LOGV("fuji", "Read byte[%d]: 0x%02X", uartBufferPos, uartBuffer[uartBufferPos]);
            uartBufferPos++;
        }
        
        // Check if we have a complete frame
        if (uartBufferPos >= 8) {
            memcpy(readBuf, uartBuffer, 8);
            ESP_LOGD("fuji", "Complete frame received via UART");
            uartBufferPos = 0;  // Reset buffer for next frame
        } else {
            // Not enough data yet
            return false;
        }
    } else if (_serial != nullptr) {
        // HardwareSerial: Use original blocking approach
        ...
        // Check if read operation took too long
        unsigned long readTime = millis() - startTime;
        if (readTime > 50) {
            ESP_LOGW("fuji", "Serial read took %lu ms, potential blocking issue", readTime);
        }

        if (bytesRead < 8) {
            ESP_LOGD("fuji", "Incomplete frame: only %d bytes read", bytesRead);
            return false;
        }
    } else {
        // No UART configured
    ...
    // Process the complete frame (common for both UART types)
    for (int i = 0; i < 8; i++)
    {
        readBuf[i] ^= 0xFF;
    }
